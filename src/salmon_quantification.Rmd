...
vsd_jcvi = DESeq2::vst(dds_jcvi, blind=TRUE)
assay(vsd_jcvi) %>% as_tibble(rownames = 'gene_id') %>% 
...
tpm_chrom_level = list.files(here('output', 'salmon_quant', 'chrom_level'), 
  ...
  purrr::reduce(full_join, by='gene_id')  %>% 
  rename_with(~str_replace(., '.*/(.*?)/quant.sf', '\\1'))
tpm_chrom_level %>% write_csv(here('output', 'salmon_quant', 'jcvi', 'A_flavus_chrom_level_tpm_unfiltered.csv'))
...
vsd_chrom_level = DESeq2::vst(dds_chrom_level, blind=TRUE)
assay(vsd_chrom_level) %>% as_tibble(rownames = 'gene_id') %>% 
...
pca_data_jcvi <- DESeq2::plotPCA(vsd_jcvi, intgroup='bioproject', returnData=T, ntop=4000)
...
```{r, fig.height=6, fig.width=10}
pca_data_chrom_level <- DESeq2::plotPCA(vsd_chrom_level, intgroup='bioproject', returnData=T, ntop=4000)
...
mpn65 = c('#ff0029','#377eb8','#66a61e','#984ea3','#00d2d5','#ff7f00','#af8d00','#7f80cd','#b3e900','#c42e60','#a65628',
          ...
          '#cbe8ff','#fecddf','#c27eb6','#8cd2ce','#c4b8d9','#f883b0','#a49100','#f48800','#27d0df','#a04a9b')
tpm_chrom_level %>% 
  filter(gene_id == 'F9C07_2278511') %>%
  pivot_longer(-gene_id, names_to='sra_run', values_to='tpm') %>%
  left_join(metadata_df %>% as_tibble(rownames='sra_run')) %>%
  ggplot(aes(sra_run, tpm, fill=bioproject)) +
  geom_col() +
  scale_fill_manual(values = mpn65) +
  ggeasy::easy_remove_legend()
...
single_gene_barplot(tpm_jcvi, 'AFLA_023020')
